...
class HuggingfaceInference(Inference):
    def __init__(self, model_path, max_new_tokens=256, temperature=0.7, top_p=0.95, top_k=1, repetition_penalty=1.15,
                 using_4bit_quantization=True, low_cpu_mem_usage=True):
        self.model = None
        self.tokenizer = None
        self.hg_model = None
    ...
    def load_model(self):
        try:
            ...
            if self.bnb_config:
                self.hg_model = AutoModelForCausalLM.from_pretrained(self.model_path, device_map="auto",
                                                                     quantization_config=self.bnb_config,
                                                                     low_cpu_mem_usage=self.low_cpu_mem_usage,
                                                                     trust_remote_code=True)
            else:
